  ...
  const processFile = async (file: File) => {
    ...
    const processAsRegularImage = async () => {
      return new Promise<string>((resolve, reject) => {
        ...
        img.onload = () => {
          console.log('Image loaded as regular image:', img.width, 'x', img.height)
    ...
    try {
      ...
      try {
        ...
        throw new Error(`GeoTIFF parsing failed: ${parseError instanceof Error ? parseError.message : 'Unknown parsing error'}`)
      }
      ...
      const convertToWGS84 = (west: number, south: number, east: number, north: number, geoKeys: any) => {
        ...
        if (Math.abs(west) > 180 || Math.abs(east) > 180 || Math.abs(south) > 90 || Math.abs(north) > 90) {
          console.log('Coordinates appear to be in projected format (UTM/other), attempting conversion...')
          ...
          try {
            ...
            if (geoKeys && (geoKeys.ProjectedCSTypeGeoKey || geoKeys.GeographicTypeGeoKey)) {
              ...
              console.log(`No EPSG found, guessing UTM zone ${utmZone}N (${sourceProj})`)
            }
            ...
            if (sourceProj) {
              ...
              const [convertedWestLon, convertedSouthLat] = proj4(sourceProj, targetProj, [west, south])
              const [convertedEastLon, convertedNorthLat] = proj4(sourceProj, targetProj, [east, north])
      ...
      if (bbox && bbox.length === 4) {
        ...
        console.log('Using bounding box coordinates:', { rawWest, rawSouth, rawEast, rawNorth })
        
        // Validate coordinates
        if (isNaN(rawWest) || isNaN(rawSouth) || isNaN(rawEast) || isNaN(rawNorth)) {
          throw new Error('Invalid coordinates in bounding box')
        }
        
        // Convert coordinates if needed
        const { west, south, east, north } = convertToWGS84(rawWest, rawSouth, rawEast, rawNorth, geoKeys)
        ...
        console.log('Calculated from origin/resolution:', { rawWest, rawSouth, rawEast, rawNorth })
      ...
      if (numBands >= 3) {
        ...
        for (let y = 0; y < canvas.height; y++) {
          for (let x = 0; x < canvas.width; x++) {
            ...
            const r = Math.min(255, Math.max(0, (rasters[0] as any)[sourceIndex] || 0))
            const g = Math.min(255, Math.max(0, (rasters[1] as any)[sourceIndex] || 0))
            const b = Math.min(255, Math.max(0, (rasters[2] as any)[sourceIndex] || 0))
        ...
        for (let y = 0; y < canvas.height; y++) {
          for (let x = 0; x < canvas.width; x++) {
            ...
            const normalized = Math.floor(((rawValue - min) / (max - min)) * 255)
      ...
      setGeoInfo(`✅ GeoTIFF processed! Bounds (WGS84): ${south.toFixed(6)}, ${west.toFixed(6)} to ${north.toFixed(6)}, ${east.toFixed(6)}`)
      ...
      setGeoInfo(`⚠️ GeoTIFF failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      ...
      try {
        ...
        const manualBounds = prompt(`Could not extract coordinates from GeoTIFF. 
...
Example for Münster area: 7.55,51.95,7.65,52.05`)
        ...
        if (manualBounds) {
          ...
          if (coords.length === 4 && coords.every(n => !isNaN(n))) {
            ...
            setGeoInfo(`📍 Manual coordinates applied: ${south}, ${west} to ${north}, ${east}`)
        ...
        setGeoInfo(`❌ Cannot process file: ${fallbackError instanceof Error ? fallbackError.message : 'Unknown error'}`)
        alert('Error: File cannot be processed. Please check if it\'s a valid image file.')
        setIsProcessing(false)
      }
    }
  }
        ...
        <div className="space-y-4">
          ...
          {showWorldFileInput && (
            <div className="border rounded p-3 space-y-2 bg-gray-50">
              <p className="text-xs text-gray-600">
                If GeoTIFF doesn't work, upload your image and its world file separately:
              </p>
              <div className="grid grid-cols-2 gap-2">
                <input
                  type="file"
              ...
              <p className="text-xs text-gray-500">
                📍 World file contains: pixel size, rotation, upper-left coordinates
              </p>
            </div>
          )}
          ...
          <div className="text-xs space-y-1">
            <p className="text-gray-500">
              🗺️ <strong>Auto-Load:</strong> Loads your new.tif automatically with coordinates
            </p>
