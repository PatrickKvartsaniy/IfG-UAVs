...
export default function SimpleTiffLoader({
  onImageProcessed,
}: SimpleTiffLoaderProps) {
  ...
  const utmToWgs84 = (easting: number, northing: number): [number, number] => {
    ...
    const Q4 =
      ((61 + 90 * T1 + 298 * C1 + 45 * T1 * T1 - 1.6 * e1sq - 3 * C1 * C1) *
        Math.pow(D, 6)) /
      720;
  ...
  const loadCompressedImage = async () => {
    ...
    try {
      ...
      alert(
        "Fehler: Komprimiertes Bild konnte nicht geladen werden. Stellen Sie sicher, dass compressed_survey_new.jpg im public Ordner liegt.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const loadWithManualCoords = () => {
    const coordsInput =
      prompt(`Bitte geben Sie die Koordinaten manuell ein (Lat/Lon Format):
...
Beispiel für Münster: 51.96,7.61,51.97,7.63`);
    ...
    if (coordsInput) {
      ...
      if (coords.length === 4 && coords.every((n) => !isNaN(n))) {
        ...
        alert(
          "Ungültige Koordinaten. Bitte verwenden Sie das Format: lat,lon,lat,lon",
        );
      }
    }
  };
