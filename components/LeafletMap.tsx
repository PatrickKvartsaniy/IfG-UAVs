...
(window as any).openStationPopup = (objectId: number) => {
  ...
  if (marker) {
    try {
      ...
      const offsetLatLng = L.latLng(markerLatLng.lat + 0.0003, markerLatLng.lng);
...
const LeafletMap = forwardRef<LeafletMapRef, LeafletMapProps>(({ 
  ...
  useImperativeHandle(ref, () => ({
    openPopup: (objectId: number) => {
      ...
      if (marker) {
        ...
        console.warn(`No marker found for objectId: ${objectId}`);
        console.warn('Available markers:', Array.from(markersRef.current.entries()));
      }
    }
  }))
  ...
  useEffect(() => {
    ...
    if (surveyBounds) {
      ...
      center = [(surveyBounds[0][0] + surveyBounds[1][0]) / 2, (surveyBounds[0][1] + surveyBounds[1][1]) / 2];
    ...
    console.log('Creating new map instance with center:', center, 'zoom:', zoom);
